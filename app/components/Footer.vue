<template>
  <footer class="py-12 text-lg text-white sm:py-20 bg-purple-progress">
    <div class="container px-5 mx-auto lg:px-8">
      <div class="grid grid-cols-1 gap-10 mb-4 sm:grid-cols-2 lg:grid-cols-5 lg:gap-6">
        <div class="sm:col-span-2 lg:col-span-2">
          <img src="/logo-footer.svg" alt="" class="mb-3" />
          <p class="font-light">Helps people execute their <br />bright ideas</p>
        </div>
        <div>
          <div class="mb-8 font-bold">Explore</div>
          <ul class="font-light">
            <li class="mb-3">Our Services</li>
            <li class="mb-3">Equity System</li>
            <li class="mb-3">Refund</li>
            <li class="mb-3">Shareholder</li>
          </ul>
        </div>
        <div>
          <div class="mb-8 font-bold">Investor</div>
          <ul class="font-light">
            <li class="mb-3">My Account</li>
            <li class="mb-3">Top Startups</li>
            <li class="mb-3">How-to Tutorials</li>
            <li class="mb-3">Withdrawal</li>
          </ul>
        </div>
        <div>
          <div class="mb-8 font-bold">Office</div>
          <ul class="font-light">
            <li class="mb-3">+021 2208 1996</li>
            <li class="mb-3">Denpasar, Bali</li>
            <li class="mb-3">No.12 (Backer)</li>
            <li class="mb-3">support@backer.com</li>
          </ul>
        </div>
      </div>
    </div>
  </footer>
</template>
